# ghostwriter
ghostwriter is a CLI tool that converts Terraform code into Professional Cloud Architecture Diagrams and solves the problem of keeping documentation up to date. With high velocity releases the norm now, machine generated architecture diagrams are more accurate than relying on the initial diagram hand drawn by the cloud architect that doesn't match reality anymore. ghostwriter securely runs 100% Client Side and dynamically parses your conditionally created resources and variables in Terraform to generate an automatic image or JSON text representation of your architecture. ghostwriter is designed to be a 'Docs as Code' (DaC) tool that can be included in your CI/CD pipeline to update architecture diagrams after your build/test/release pipeline phases. It currently Supports AWS and soon Google and Azure cloud.
...
1. Cost
	- Save Visio/Drawing software licenses - ghostwriter is free and open source
...
2. Accelerate and Automate
	...
	- Automate creation of architecture diagrams by running ghostwriter as part of CI/CD pipelines
...
2. Clone the repo ``git clone https://github.com/patrickchugh/ghostwriter.git``
3. Get the working directory (full path) by doing `cd ghostwriter` and doing `pwd`.
4. Add the ghostwriter folder to your `PATH` value e.g. ``export PATH=$PATH:/Users/<PATH TO TERRAFORM>`` so you can run it from anywhere. `<PATH TO TERRAFORM>` would be the output from line 3.
5. Install python requirements ``cd ghostwriter && pip install -r requirements.txt``
6. Ensure your ghostwriter python script is executable with ``chmod +x ghostwriter``
7. Run `ghostwriter` and specify your Terraform source files in the format:
``` bash
$ ghostwriter draw --source ~/src/my-terraform-code
```

For Terraform source code in a Git repo you can also use the form:
``` bash
$ ghostwriter draw --source https://github.com/your-repo/terraform-examples.git
```
Use the `//` character for subfolders within Git repos if the code you want is under a folder hierarchy.
``` bash
$ ghostwriter draw --source https://github.com/your-repo/terraform-examples.git//mysubfolder/secondfolder/
```
# Example Terraforms to try
Unrelated to my project, but here are a few third-party Terraform examples to try:
``` bash

ghostwriter draw --source https://github.com/futurice/terraform-examples.git//aws/aws_static_site --varfile examples/variables.tfvars --show

ghostwriter draw --source https://github.com/futurice/terraform-examples.git//aws/wordpress_fargate --varfile examples/variables.tfvars --show

ghostwriter draw --source https://github.com/k-mitevski/terraform-k8s.git//01_terraform_eks --show


```

# Annotating generated diagrams
No automatically generated diagram is going to have all the detail you need, at best it will get you 80-90% of the way there. To add custom annotations such as a main diagram title, additional labels on arrows or additional resources created outside your Terraform, include a new file `ghostwriter.yml` file in the source code folder and it will be automatically parsed by ghostwriter. Alternatively, specify a path to the annotations file as a parameter to ghostwriter. 

``` bash
ghostwriter --source https://github.com/your-repo/terraform-examples.git --annotate /Users/me/MyDocuments/annotations.yml
```

The .yml file is a standard YAML configuration file that is similar to the example below with one or more headings called `title`, `connect`, `disconnect`, `add`, `remove` or `update`. The node names follow the same conventions as Terraform resource names https://registry.terraform.io/providers/hashicorp/aws/latest/docs and support wildcards. You can add a custom label to any TF resource by modifying the attributes of the resource and adding the `label` attribute (doesn't exist in Terraform). For lines/connections, you can modify the resource attributes by adding ghostwriter specific `edge_labels` to add text to the connection line to a specific resource node. See the example below:
...
Type ``ghostwriter --help`` for full command list or for help with a specific command
